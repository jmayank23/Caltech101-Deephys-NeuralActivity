...
4. **Visualization for each image:** Shows the most activated neurons for that image along with other images that the neuron was also activated for. Also shows ground truth and predictions for that image by the model.
...
5. **Visualization for each neuron:** Presents the images a given neuron was highly activated for (arranged in decreasing order of activation value.)
